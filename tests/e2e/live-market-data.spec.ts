...
test("personal portfolio searches live universe, backfills history and updates all analytics", async ({ page }) => {
  ...
  expect(dialogWidth).toBeGreaterThanOrEqual(720);
  expect(dialogWidth).toBeLessThanOrEqual(900);
  ...
  await page.getByLabel("Datum nákupu", { exact: true }).fill("2026-09-10");
  await page.getByRole("button", { name: "Zkontrolovat investici" }).click();
  ...
  await page.getByLabel("Hledat akcii nebo ETF").fill("Apple");
  await page.getByRole("option", { name: /AAPL Apple Inc/ }).click();
  await expect(page.getByText(/AAPL už je ve vašem portfoliu/)).toBeVisible();
  await page.getByLabel("Množství", { exact: true }).fill("1");
  await page.getByLabel("Nákupní cena", { exact: true }).fill("205");
  await page.getByLabel("Datum nákupu", { exact: true }).fill("2026-09-11");
  await page.getByRole("button", { name: "Zkontrolovat investici" }).click();
  ...
  await page.getByRole("button", { name: "Hledat na trhu" }).click();
  await page.locator(".compare-market-search").getByLabel("Hledat akcii nebo ETF").fill("Vanguard");
  await page.locator(".compare-market-search").getByRole("option", { name: /VWCE Vanguard FTSE All-World/ }).click();
