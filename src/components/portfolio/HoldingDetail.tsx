...
export function HoldingDetail({ holding: p, locale, onClose, onCompare, onAdd }: { holding: HoldingMetric; locale: string; onClose: () => void; onCompare: () => void; onAdd: () => void }) {
  ...
  const trades = transactions.filter((row): row is Trade => "assetId" in row && row.assetId === p.assetId).sort((a, b) => b.occurredAt.localeCompare(a.occurredAt));
  ...
  return <Dialog open wide title={p.asset.name} onClose={onClose}>
    <div className="holding-detail">
      ...
      {mode === "personal" && <section className="transaction-history"><h3>Historie transakcí <span>{trades.length}</span></h3>{trades.map((trade) => <div key={trade.id} className="transaction-row"><span><strong>{trade.type === "buy" ? "Nákup" : "Prodej"}</strong><small>{dateLabel(trade.occurredAt, locale)}</small></span><span>{trade.quantity.toLocaleString(locale)} {p.asset.symbol} @ {trade.unitPrice.toLocaleString(locale)} {trade.currency}</span><span><button onClick={() => startEditor(trade.type, trade)}>Upravit</button><button onClick={() => { removeTransaction(trade.id); setNotice("Transakce byla odstraněna."); }}>Odstranit</button></span></div>)}</section>}
